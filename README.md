...
核心只有一个 [SKILL.md](SKILL.md)。没有运行服务，不需要本项目的 API key；装进你已有的 AI 工具使用。模型访问、费用和数据处理仍按原工具设置。
...
需要 Node.js；按提示选择已有的 AI 工具。也可以[下载精简 ZIP 手动安装](#安装)。

## 看看怎么改

![Humanizer-zh 的一次真实改写：删除空泛的更新评价，保留仅网页版开放与手机端尚未支持的条件。](assets/demo/before-after.gif)

[观看或下载 34 秒演示](assets/demo/humanizer-zh.mp4) · [静态对照](assets/demo/comparison.png) · [完整输入、输出与运行依据](docs/DEMO.md)
...
套话删掉了，使用范围还在。另一条技术说明保留了全部数字和未验证事项；一段本来就自然的话则原样保留，见[三个实际例子](docs/DEMO.md)。

演示使用 **1.1.0** 与 gpt-6-astra，每个自拟例子运行一次，输出未经人工改写。视频根据保存的结果排版，节奏经过剪辑。它展示本次观察到的行为，不是统计评测或其他模型的效果保证。
